...
class SoftmaxRegression(object):
    ...
    def __init__(self, input_size, num_classes, lamda):
        ...
        self.theta = 0.005 * numpy.asarray(rand.normal(size=(num_classes *
                                                             input_size, 1)))
    ...
    def softmaxCost(self, theta, input, labels):
        ...
        theta_grad = -input.dot((ground_truth - probabilities).T)
        theta_grad = theta_grad / input.shape[1] + self.lamda * theta.T
        theta_grad = numpy.array(theta_grad)
        theta_grad = theta_grad.flatten('F')
...
def executeSoftmaxRegression():
    ...
    opt_solution = scipy.optimize.minimize(regressor.softmaxCost,
                                           regressor.theta,
                                           args=(training_data,
                                                 training_labels, ),
                                           method='L-BFGS-B',
